...
MATCH_THRESHOLD    = 0.60                   # Euclidean distance threshold (lower = stricter)
MISMATCH_THRESHOLD = 0.70                   # Hysteresis: mismatch → verified needs < MATCH_THRESHOLD
...
def main(ref_dir: str):
    ...
    cached_distance   = 0.0
    hysteresis_match  = True     # hysteresis state: prevents flickering
    ...
    while True:
        ...
        if frame_idx % RECOG_EVERY == 0:
            ...
            if cached_locations:
                ...
                if encs:
                    ...
                    # Hysteresis: stay in current state unless threshold is clearly crossed
                    if hysteresis_match:
                        # Currently verified: need dist > MISMATCH_THRESHOLD to switch to mismatch
                        cached_match = dist < MISMATCH_THRESHOLD
                        if not cached_match:
                            hysteresis_match = False
                    else:
                        # Currently mismatch: need dist < MATCH_THRESHOLD to switch to verified
                        cached_match = dist < MATCH_THRESHOLD
                        if cached_match:
                            hysteresis_match = True
                else:
                    cached_match    = False
                    cached_distance = 1.0
            else:
        ...
        cv2.putText(frame, f"match_thresh={MATCH_THRESHOLD} mismatch_thresh={MISMATCH_THRESHOLD}",
                    (w - 320, h - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, WHITE, 1)
